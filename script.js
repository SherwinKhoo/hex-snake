...
const calculateHexesToSides = () => {
  const gridShortSide = Math.floor(
    Math.min(usableCanvasHeight, usableCanvasWidth) / hexHeight
  );
  const gridLongSide = Math.floor(
    Math.max(usableCanvasHeight, usableCanvasWidth) / hexHeight
  );
...
const calculateHexCentreFromGridCentre = (axialCoordinates) => {
  ...
  return [
    xCentre + hexRadius * (sqrt3 * q + 0.5 * sqrt3 * r),
    yCentre + hexRadius * (1.5 * r),
  ]; // pixels
};
...
const renderGameObject = (
  ...
  for (const value of gameObject.values()) {
    ...
    if (gameObject !== myFood) {
      renderHex(
        calculateHexCentreFromGridCentre(value),
        hexRadius,
        isSolid,
        pixelColour
      );
...
const handleClicksss = (action) => {
  setTimeout(() => {
    action();
  }, 1);
};

buttonCentre.addEventListener("click", () => handleClicksss(handleStartReset));
buttonLeft.addEventListener("click", () => handleClicksss(handleLeftTurn));
buttonRight.addEventListener("click", () => handleClicksss(handleRightTurn));
